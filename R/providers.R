...
.detect_provider <- function(model) {
    ...
    if (!is.null(base)) {
        if (grepl("moonshot|kimi", base, ignore.case = TRUE)) {
            return("moonshot")
        }
    ...
    if (!is.null(model)) {
        ...
        if (grepl("^kimi|^moonshot-v1", model, ignore.case = TRUE)) {
            return("moonshot")
        }
        if (grepl("^(llama|mistral|phi|qwen|gemma|codellama|deepseek)", model,
                  ignore.case = TRUE)) {
            return("ollama")
        }
...
#' }
chat_claude <- function(prompt, model = "claude-sonnet-4-6", ...) {
